...
Godot's `Curve3D.bake_interval` controls the distance between cached baked curve points inside the `Curve3D` resource. ProtoWall leaves that value alone. In `Follow Curved Path3D`, enable `follow_use_bake_interval` when you want bake interval to drive generated sample density; lower bake interval values place fewer generated samples and higher values place more generated samples using cubic `sample_baked()` interpolation. When enabled, `path_sample_spacing` is ignored.
