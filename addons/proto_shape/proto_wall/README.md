...
- `follow_use_bake_interval` - `Follow Curved Path3D` only. Uses `Curve3D.bake_interval` directly as the spacing between generated samples and ignores `path_sample_spacing`.
...
Godot's `Curve3D.bake_interval` controls the distance between cached baked curve points inside the `Curve3D` resource. ProtoWall leaves that value alone. In `Follow Curved Path3D`, enable `follow_use_bake_interval` to use that interval directly between generated samples: lower values create more sections and higher values create fewer. When enabled, `path_sample_spacing` is ignored.
