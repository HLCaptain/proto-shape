...
func _get_dragged_value(handle_id: int, camera: Camera3D, screen_pos: Vector2) -> Variant:
	match handle_id:
		HANDLE_RADIUS:
			var offset: Variant = gizmo_utils.get_handle_offset(camera, screen_pos, _get_radius_handle_position(), Vector3.RIGHT, self)
			return offset.x if offset is Vector3 else null
		HANDLE_HEIGHT:
			var offset: Variant = gizmo_utils.get_handle_offset(camera, screen_pos, _get_height_handle_position(), Vector3.UP, self)
