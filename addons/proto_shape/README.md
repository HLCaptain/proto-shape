...
`ProtoGizmo` is a reusable `EditorNode3DGizmoPlugin`. Custom `Node3D` shapes can expose a provider through `get_proto_gizmo_provider()` or use `ProtoGizmoWrapper` as a signal bridge for child nodes. Providers can combine point handles with selectable arrow-body subgizmos for larger 3D drag targets.
...
The gizmo is an `EditorNode3DGizmoPlugin` and is visible when a node with a gizmo provider or a child of `ProtoGizmoWrapper` is selected. Selection highlights the mesh with a transparent blue color and shows handles and solid arrows, which you can drag or transform to adjust the shape.
